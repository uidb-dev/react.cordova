_284‍.x([["cli",()=>cli]]);const fs = require("fs");
...
// });
    ...
    const jsonfile = require('jsonfile');
    const file = './bin/localStorage.json';
    jsonfile.readFile(file)
        .then(obj => {
            ...
            if(!obj.installReco){
                require('child_process').exec(
                    ...
                    , function (error, stdout, stderr) {
                        if (error) {
                            console.error('ERROR recoLink: ' + error);
                            return;
                        } else {
                            console.log("welcome to react.cordova: reco cli.")
                            console.log("try to run =>  reco init <com.yourAppId> <yourAppName>")
                        }
                    }).on('data', (data) => {
                        console.log("data recoLink: OK", data.toString());
                    });

                    obj.installReco=true;
                    jsonfile.writeFile(file, obj, function (err) {
                        if (err) {
                            console.error("ERROR: local storege and link cli: ", err);
                            return;
                        }
                    })
            }
