...
`);
    ...
    // console.log();
    reco.state.child_process.exec("cordova help", function (error, stdout, stderr) {
      stdout = stdout.replace(" create ............................. Create a project", "");
      console.log(stdout.slice(0, stdout.indexOf("cordova command [options]")));
      console.log(colors.underline.green(stdout.slice(stdout.indexOf("cordova command [options]"), stdout.indexOf("cordova command [options]") + 25)));
    ...
    if (fs.existsSync("./www")) {
      ...
      ncp(parentDir + buildFolderName, parentDir + "www", function (err) {
        if (err) {
          ...
          return console.error("ERROR ncp2, copy " + buildFolderName + " to www :   " + err);
        }
        reco.state.callBack_replaceWwwRootDir(); // callBack();
      });
    }
  ...
  version: (automatic, fromBuild) => {
    try {
      reco.state.child_process.exec("npm view react.cordova --json", function (error, stdout, stderr) {
        if (error) {
          reco.setState({
            error: true
          });
        ...
        //   ) + "package.json";
        reco.state.child_process.exec("npm list -g", {
          maxBuffer: 5120 * 5120
          ...
          let newVersion;
          const thisVersion = obj.slice(obj.lastIndexOf("react.cordova@") + 14, obj.lastIndexOf("react.cordova@") + 19);
          if (dataView && dataView.latest && Array.isArray(dataView.latest)) newVersion = dataView?.latest[dataView?.versions?.length - 1];
          //dataView["dist-tags"].latest;
          else if (dataView && dataView["dist-tags"] && dataView["dist-tags"].latest) newVersion = dataView["dist-tags"].latest;else return;
